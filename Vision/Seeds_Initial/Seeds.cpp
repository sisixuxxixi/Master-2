...
static float correl(const Image<byte>& im1, int i1,int j1,float m1,
    ...
    for(int x=-win; x<=win; x++){
        for (int y=-win; y<=win; y++){
            var_im1 += pow(im1(i1+x, j1+y)-m1, 2);
            var_im2 += pow(im2(i2+x, j2+y)-m2, 2);
            dis += (im1(i1+x, j1+y)-m1) * (im2(i2+x, j2+y)-m2);
        }
    }
    dist = dis/(sqrt(var_im1) * sqrt(var_im2));
    return dist;
}
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            for (int d=dmin; d<=dmax; d++) {
                ...
                float cor = ccorrel(im1,x,y,im2,x+d,y);
                // be better
                if (cor > max_ncc){
                    best_d = d;
                    max_ncc = cor;
            ...
            if (max_ncc > nccSeed) {
                seeds(x,y) = true;
                disp(x,y) = best_d;
                Seed new_seed = Seed(x,y,best_d,max_ncc);
                Q.push(new_seed);
            }
        }
    }
...
static void propagate(Image<byte> im1, Image<byte> im2,
            ...
            if(0<=x-win && x+win<im1.width() && 0<=y-win && y+win<maxy &&
               ! seeds(x,y)) {
                ...
                for (int d=1; d>=-1; d--){
                    ...
                    float new_cor = ccorrel(im1,x,y,im2,x+total+d,y);
                    if(new_cor > best_cor){
                        best_d = total + d;
                        best_cor = new_cor;
                    }
                ...
                seeds(x,y) = true;
                disp(x,y) = best_d;
                Seed new_seed = Seed(x,y,best_d,best_cor);
                Q.push(new_seed);
            }
        }
    }
    ...
    save(displayDisp(disp,W,4), srcPath("2propagate.png"));
