...
//const float lambdaf = 1;
//const float lambdaf = 0.05;
...
void build_graph(Graph<int,int,int>& G,
    ...
    for (int x=0; x<nx; x++){
        for (int y=0; y<ny; y++){
            ...
            int penalty = 1 + (nd - 1) * lambda * nb_neighbor;
            
            for (int d=0; d<nd; d++){
                float wight = INF;
                if ( (zoom * x + win + dmin + d > win) && (zoom * x + win + dmin + d < I2.width() - win) ){
                    float value_zncc = zncc(I1, I1M, I2, I2M, zoom * x + win, zoom * y + win, zoom * x + win + dmin + d, zoom * y + win);
                    if (value_zncc < 0){
                        value_zncc = 1;
                    }else{
                        value_zncc = sqrt(1 - value_zncc);
                    }
                    wight = wcc * value_zncc + penalty;
                }
                
                if (d != 0){
                    G.add_edge(y + ny * (x + nx * (d - 1)), y + ny * (x + nx * d), wight, 0);
                } else {
                    G.add_tweights(y + ny * (x + nx * d), wight, 0);
                }
                if (d == nd - 1){
                    G.add_tweights(y + ny * (x + nx * d), 0, wight);
                }
                
                if(x < nx - 1){
                    G.add_edge(y + ny * (x + nx * d), y + ny * (x + 1 + nx * d), lambda, lambda);
                }
                if(y < ny - 1){
                    G.add_edge(y + ny * (x + nx * d), y + 1 + ny * (x + nx * d), lambda, lambda);
                }
            }
        }
    }
...
doubleImage decode_graph(Graph<int,int,int>& G, int nx, int ny, int nd) {
    ...
    for(int x=0; x<nx; x++) {
        for(int y=0; y<ny; y++){
            int d = 0;
            while ( (d < nd) && (G.what_segment(y + ny * (x + nx * d)) == Graph<int,int,int>::SOURCE) ){
                d++;
            }
            D(x,y) = d + dmin;
        }
